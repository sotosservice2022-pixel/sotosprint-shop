...
function quota429Message(data) {
  ...
  // Google не уточнив тип. Евристика: коротка затримка (≤2 хв) — хвилинний ліміт;
  // немає затримки або велика — майже завжди ДЕННИЙ (≈20/день). Це прибирає плутане «або/або».
  if (retrySec > 0 && retrySec <= 120) {
    return `Ліміт запитів на хвилину (близько 5/хв). Зачекай ~${retrySec} с і спробуй ще.${rawTail}`;
  }
  return `Схоже, вичерпано ДЕННИЙ безкоштовний ліміт Gemini (≈20 запитів/день).${dailyWhen} Кожне натискання «Скласти з фото» = 1 запит. Або підключи біллінг у Google AI Studio, щоб підняти ліміт.${rawTail}`;
}
